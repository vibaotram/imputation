...
sudo docker build -t vibaotram/imputation:1.3 .
sudo docker push vibaotram/imputation:1.3
sudo docker run -it vibaotram/imputation:1.3
...
snakemake --sdm conda apptainer -F
snakemake ACCURACY --sdm conda apptainer --configfile config.yaml --rerun-triggers mtime -np 
snakemake ACCURACY --sdm conda apptainer --configfile config.yaml --report report.html
